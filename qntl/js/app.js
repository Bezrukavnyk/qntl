...
//=================
//search
let searchMenu = document.querySelector(".header_search");
if (searchMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".search__body");
	...
	for (let index = 0; index < tabs_items.length; index++) {
		...
		tabs_item.addEventListener("click", function (e) {
			if(!tabs_item.classList.contains('_active')) {
				...
				e.preventDefault();
			}else if(tabs_item.classList.contains('_active')){
				tabs_item.classList.remove('_active');
				tabs_blocks[index].classList.remove('_active');
			}
		});
...
function popup_close(item, bodyUnlock = true) {
	...
	document.querySelector('.wrapper').classList.remove('form')
}
...
let scrItems = document.querySelectorAll('.four__point');
...
// let callForm = document.querySelectorAll('._tell-form');
// callForm.forEach((e)=>{
//     e.addEventListener('click', ()=>{
//       document.querySelector('.popup_popup').classList.add('_active')
//       document.querySelector('.wrapper').classList.add('form')
//     })
// })
// video player magic overlay
let searchActive = document.querySelector('.icon-menu')
searchActive.addEventListener('click', ()=>{
    if (document.querySelector('.header_search').classList.contains('_active')){
        document.querySelector('.header_search').classList.remove('_active')
        document.querySelector('.search__body').classList.remove('_active')

    }
})
let menuActive = document.querySelector('.header_search')
...
let newsSingle = new Swiper(".news-slider", {
    pagination: {
...
if(window.innerWidth<1050){
    document.querySelectorAll('.main__desc--item').forEach((e)=>{
        e.classList.add('swiper-slide')
    })
    let myImageSlider = new Swiper('.main__desc', {
        navigation: {
...
let swiper = new Swiper(".roomSwiper", {
    slidesPerView: 'auto',
    loop: true,
    centeredSlides: true,
    spaceBetween: 10,
    pagination: {
        el: ".roomSwiper-navigation",
        dynamicBullets: true,
    },
    breakpoints: {
        640: {
...
let swiperInclud = new Swiper(".includ", {
    slidesPerView: 6,
    loop: true,
...
let sliderFooter = new Swiper(".slider--footer", {
    slidesPerView: 2,
    freeMode: true,
...
document.querySelectorAll('.go-basket').forEach((e)=>{
e.addEventListener('click', ()=>{
...
if(window.innerWidth < 880){
    let linkOut = document.querySelectorAll('.sub__parrent');
    ...
    async function formSend(e) {
        ...
        if (error === 0) {
            ...
            if (response.status == 200){
                ...
                document.querySelector('.popup_popup').classList.remove('_active')
                document.querySelector('.popup_popup').classList.remove('_sending')
                form.reset();
                form.classList.remove('_sending');
            }else {
...
let inp = document.querySelector('.btn-form');
...
if(document.querySelector('.about__wraper._services')) {
    const items = document.querySelectorAll('.about__points');

    if (items.length !== 0) {

        items.forEach((e) => {
            function img(){
                let id = e.getAttribute("data-img-value");
                document.querySelectorAll('.about__points').forEach((elem) => {
                    elem.classList.remove("active")
                })
                e.classList.add("active")
                document.querySelectorAll('img[data-img]').forEach((elem) => {
                    elem.classList.remove("active")
                })

                document.querySelectorAll(`img[data-img="${id}"]`).forEach((e) =>                  {
                    e.classList.add("active");
                })
            }
    ...
    document.querySelector('.about__points').click();
}
